...
build_cell_plot <- function(run_data, N, M, alpha, design) {
  ...
  ggplot() +
    # Panel background: green if significant, light gray if not
    geom_rect(
      data = summ,
    ...
    # p and d labels in top-right of each panel
    geom_text(
      data    = summ,
      aes(label = p_label),
      ...
      for (rep in seq_len(C)) {
        ...
        if (design == "between") {
          ...
          for (i in seq_len(N)) {
            ...
            sb         <- subj_ability[i] - item_diff[b_idx] + rnorm(M_b, sd = E) + raw_effect
            scores_a[i] <- mean(sa)
            scores_b[i] <- mean(sb)
          }
          p <- t.test(scores_b - scores_a)$p.value
        }
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      helpText("SD of the subject ability distribution.",
               "Leave at 1 unless you want to emphasise or suppress between-subject variability."),

      numericInput("M_weight", "Item variability weight",
                   value = 1, min = 0.1, max = 10, step = 0.1),
